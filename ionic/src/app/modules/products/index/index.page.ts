...
export class IndexPage implements OnInit {
    ...
    async paginate(ionRefresher: any = null, ionInfiniteScroll: any = null) {
        ...
        this.productService.get([], this.getFilters(), this._paginate)
            .then(
                async (response: any) => {
                    this.total_of_data = response.products.total;
                    this._paginate.page = response.products.current_page + 1;
                    ...
                    if (this.total_of_data == this.products.length) {
    ...
    async remove(user: Product, eventPopover: any) {
    ...
    destroy(model: Product) {
        this.productService.destroy(model.id)
            .then((response) => {
                if (!response.error) {
                    var index = this.products.findIndex(ac => ac.id == model.id);
                    if (index > -1) {
